...
float prob_given2(int x,int attrib,vector<vector<int> > D){
    int fav_widx=0;
    int fav=D[0].size();
    for(int i=0;i<D[0].size();i++){
        if(D[attrib][i]==x){
...
float new_prob(vector<int> tup,vector<vector<int> > D){
    float ans=1.000000;
    for(int i=0;i<tup.size();i++){
        ans*=prob_given2(tup[i],i,D);
    }
    return ans;
}
